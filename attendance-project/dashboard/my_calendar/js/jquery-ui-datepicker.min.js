...
function updateSessionInfoByDay() {
    month = ["JANUARY","FEBRUARY","MARCH","APRIL","MAY","JUNE","JULY","AUGUST","SEPTEMBER","OCTOBER","NOVEMBER","DECEMBER"]
    selectedDay = document.getElementsByClassName('ui-state-active')[0].innerHTML;
    selectedMonth = document.getElementsByClassName('ui-datepicker-month')[0].innerText;
    selectedYear = document.getElementsByClassName('ui-datepicker-year')[0].innerText;
    selectedMonth = (month.indexOf(selectedMonth)+1).toString();
    console.log(selectedDay,selectedMonth,selectedYear);
    date = selectedYear+"-"+selectedMonth+"-"+selectedDay;
    mydate = new Date(selectedYear,selectedMonth-1,selectedDay);

    cal_getTotalStudent(date);
    cal_getSubject(mydate.getDay())
}
...
function cal_getSubject(weekday) {
    let obj ={
        "day_id":weekday,
    ...
    let morSubEll =document.getElementsByClassName("msu");
    let aftSubEll = document.getElementsByClassName("fsu");
    request.onreadystatechange = function () {
        if(this.readyState==4&&this.status==200){
            let objs = JSON.parse(this.responseText);


            for(let i =0;i<objs['morning'].length;i++){
                morSubEll[i].innerHTML=objs['morning'][i];
            }
            for(let i =0;i<objs['afternoon'].length;i++){
                aftSubEll[i].innerHTML=objs['afternoon'][i];
            }

            for(let i =objs['morning'].length;i<morSubEll.length;i++){
                morSubEll[i].innerHTML="";

            }
            for(let i =objs['afternoon'].length;i<aftSubEll.length;i++){
                aftSubEll[i].innerHTML="";

            }
        }
        if(this.status){
            for(let i =0;i<morSubEll.length;i++){
                morSubEll[i].innerHTML="";

            }
            for(let i =0;i<aftSubEll.length;i++){
                aftSubEll[i].innerHTML="";
    ...
    request.open("POST","../../php/schedule/schedule_readWithWeekday.php",true);
    request.send(jsondata);
}


function cal_getTotalStudent(date){
    let class_id = getCookie("class_id");
    let request = new XMLHttpRequest();
    request.onreadystatechange = function(){
        if(this.readyState==4&&this.status==200){
            let obj = JSON.parse(this.responseText);
            getCalendarData(date,obj.total)
...
function getCalendarData(date,total) {
    ...
    request.onreadystatechange = function () {
        if (this.readyState==4&&this.status==200){
            ...
            let morning = parseInt((objs[0].total*100)/total);
            let afternoon = parseInt((objs[1].total*100)/total);

            $('.chart1').data('easyPieChart').update(morning);
            $('span', $('.chart1')).text(morning);

            $('.chart2').data('easyPieChart').update(afternoon);
            $('span', $('.chart2')).text(afternoon);
    ...
    request.open('POST',`../../php/attendance/attendance_getOneData.php`);
    request.send(jsondata);
}
